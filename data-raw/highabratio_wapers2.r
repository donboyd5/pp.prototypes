...
# Construct young (immature) prototype, based primarily on WA-PERS2
#
...
range <- "A5:R29"  # include column headers, but NOT row or column totals
...
range <- "A4:E28"  # include column headers; for retirees, include column totals but not row totals
...
#****************************************************************************************************
range <- "A8:B39"  # include column headers
...
# These data are on a yos basis. Convert to age basis (20:70) assuming entrance at 20
df2 <- df %>% mutate(age=as.integer(20:50),
                     rate=cton(rate)) %>%
  select(age, rate)
