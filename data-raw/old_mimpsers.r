...
range <- "A5:L30"  # include column headers, but NOT row or column totals
...
#****************************************************************************************************
range <- "A4:B14"  # include column headers
...
# we can see that the rate is 0.035 from age 60 on, so force that rather than let the spline make it different
(df2 <- rbind(filter(df, age<60), data.frame(age=60:70, rate=0.035)))
